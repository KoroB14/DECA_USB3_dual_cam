...
CyU3PDmaChannel GPIF2USB_Handle1, GPIF2USB_Handle2, USB2GPIF_Handle;
...
CyBool_t USBSetup_Callback(uint32_t setupdat0, uint32_t setupdat1)
{
    ...
    if (Setup.Type == STANDARD_REQUEST)
    {
                ...
                && (Setup.Value == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplicationActive)
            {
                if (Setup.Index == USB_CONSUMER_ENDPOINT1)
                {
																  
										 
										 
                    CyU3PDmaChannelReset (&GPIF2USB_Handle1);
                    CyU3PUsbFlushEp(USB_CONSUMER_ENDPOINT1);
                    CyU3PUsbResetEp (USB_CONSUMER_ENDPOINT1);
                    CyU3PDmaChannelSetXfer (&GPIF2USB_Handle1, 0);
                }
                
                if (Setup.Index == USB_CONSUMER_ENDPOINT2)
                {
																  
										 
										 
                    CyU3PDmaChannelReset (&GPIF2USB_Handle2);
                    CyU3PUsbFlushEp(USB_CONSUMER_ENDPOINT2);
                    CyU3PUsbResetEp (USB_CONSUMER_ENDPOINT2);
                    CyU3PDmaChannelSetXfer (&GPIF2USB_Handle2, 0);
                }
...
// USB has been enumerated, start application
{
	...
	Status = CyU3PSetEpConfig(USB_CONSUMER_ENDPOINT1, &epConfig);
	CheckStatus(Status);
	...
	dmaConfig.prodSckId		 = GPIF_PRODUCER_SOCKET1;
	dmaConfig.consSckId		 = USB_CONSUMER_ENDPOINT1_SOCKET;
	dmaConfig.dmaMode        = CY_U3P_DMA_MODE_BYTE;
	Status = CyU3PDmaChannelCreate(&GPIF2USB_Handle1, CY_U3P_DMA_TYPE_AUTO, &dmaConfig);
	CheckStatus(Status);
    

	Status = CyU3PUsbFlushEp(USB_CONSUMER_ENDPOINT1);
	CheckStatus(Status);

	// Start the DMA Channel with transfer size to Infinite
	Status = CyU3PDmaChannelSetXfer(&GPIF2USB_Handle1, 0);
	CheckStatus(Status);
    
    // Setup and flush the consumer endpoint
	Status = CyU3PSetEpConfig(USB_CONSUMER_ENDPOINT2, &epConfig);
	CheckStatus(Status);
	...
	dmaConfig.size           = DMA_BUFFER_SIZE;			// Use same size buffers for all USB Speeds
	dmaConfig.count          = DMA_BUFFER_COUNT;
	dmaConfig.prodSckId		 = GPIF_PRODUCER_SOCKET2;
	dmaConfig.consSckId		 = USB_CONSUMER_ENDPOINT2_SOCKET;
	dmaConfig.dmaMode        = CY_U3P_DMA_MODE_BYTE;
	Status = CyU3PDmaChannelCreate(&GPIF2USB_Handle2, CY_U3P_DMA_TYPE_AUTO, &dmaConfig);
	CheckStatus(Status);
    

	Status = CyU3PUsbFlushEp(USB_CONSUMER_ENDPOINT2);
	CheckStatus(Status);

	// Start the DMA Channel with transfer size to Infinite
	Status = CyU3PDmaChannelSetXfer(&GPIF2USB_Handle2, 0);
	CheckStatus(Status);
...
// USB connection has been lost, stop the application
{
    ...
    CheckStatus(Status);
    Status = CyU3PDmaChannelDestroy(&GPIF2USB_Handle1);
    CheckStatus(Status);
    Status = CyU3PDmaChannelDestroy(&GPIF2USB_Handle2);
    CheckStatus(Status);
...
CyU3PReturnStatus_t StartGPIF(void)
{
	...
	CheckStatus(Status);
	Status = CyU3PGpifSocketConfigure(0, GPIF_PRODUCER_SOCKET1, 4, CyFalse, 1); // set watermark
	CheckStatus(Status);
	Status = CyU3PGpifSocketConfigure(1, GPIF_PRODUCER_SOCKET2, 4, CyFalse, 1); // set watermark
	CheckStatus(Status);
	Status = CyU3PGpifSocketConfigure(2, GPIF_CONSUMER_SOCKET, 3, CyFalse, 1); // set watermark
	CheckStatus(Status);
